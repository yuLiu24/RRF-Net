...
int myrandom2 (int i) { return caffe_rng_rand()%i;}
...
template <typename Dtype>
void RankLossLayer<Dtype>::set_mask(const vector<Blob<Dtype>*>& bottom)
{
	...
	for(int i = 0; i < img_num; i ++)
	{
		...
		std::random_shuffle(sid1.begin(), sid1.end(), myrandom2);
		...
		std::random_shuffle(sid2.begin(), sid2.end(), myrandom2);
...
void RankLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	...
	for(int i = 0; i < img_num; i ++)
	{
		for(int j = 0; j < text_num; j ++)
		{
			...
			Dtype tloss1 = max(Dtype(0), dis_data[i * text_num + i] - dis_data[i * text_num + j] + Dtype(margin));
                         
                        // another loss: (img, text_pos) and (text_pos, text_neg)
  			Dtype ts = 0;
			...
			Dtype tloss2 = pair_size * max(Dtype(0), dis_data[i * text_num + i] - (-ts) + Dtype(margin));
	...
	for(int i = 0; i < img_num; i ++)
	{
		...
		for(int j = 0; j < text_num; j ++)
		{
			...
			if(tloss2 > 0)
			{
			     for(int k = 0; k < text_dim; k ++)
			     {
				 fori_diff[k] += -pair_size * fpos[k]; // / (pairNum * 1.0 - 2.0);
				 fpos_diff[k] += pair_size * (fneg[k]-fori[k]); // / (pairNum * 1.0 - 2.0);
				 fneg_diff[k] += pair_size * fpos[k];
			     }
			}
